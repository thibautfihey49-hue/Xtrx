...
export default function DiaryBook3D() {
  ...
  const coverStyle = useAnimatedStyle(() => ({
    transform: [
      { perspective: 800 },
      { rotateY: `${interpolate(progress.value, [0, 1], [0, -135])}deg` },
    ],
  }));
  ...
  return (
    <Pressable onPress={toggle}>
      <View style={{ width: 200, height: 280, justifyContent: 'center', alignItems: 'center' }}>
        ...
        <View style={{ position: 'absolute', width: 192, height: 272, backgroundColor: '#f5e6c8', borderRadius: 12, left: 6, top: 4 }} />
        <Animated.View style={[{ width: 200, height: 280, backgroundColor: '#FF6EC7', borderRadius: 20, justifyContent: 'center', alignItems: 'center', borderWidth: 4, borderColor: '#FFD700', zIndex: 10 } as any, coverStyle]}>
          ...
          <Text style={{ color: '#FFD700', fontWeight: 'bold', marginTop: 10, fontSize: 18 }}>HUNTR/X</Text>
          <Text style={{ color: 'rgba(255,255,255,0.8)', fontSize: 11, marginTop: 4 }}>{open ? 'tap pour fermer' : 'tap pour ouvrir'}</Text>
        </Animated.View>
        <View style={{ position: 'absolute', width: 190, height: 260, backgroundColor: '#0D0820', borderRadius: 12, justifyContent: 'center', alignItems: 'center', left: 10 }}>
          ...
          <Text style={{ color: 'rgba(255,255,255,0.5)', fontSize: 10, marginTop: 10, textAlign: 'center' }}>Ton journal est{'\n'}protégé par la{'\n'}barrière Honmoon</Text>
        </View>
      </View>
    </Pressable>
  );
